...
bData <- reactive({
    breathfileName <- paste0(here::here(),"/appData/","breath_",input$fileID,".csv")
...
cvData <- reactive({
  cvfileName <- paste0(here::here(),"/appData/","beat_",input$fileID,".csv")
...
observeEvent(input$clearReset, {
  rxVals$boutOut <-  data.frame(matrix(0,nrow = isolate(nrow(interpData())),
                                            ncol = isolate(ncol(interpData())), byrow = FALSE))
  colnames(rxVals$boutOut) <- colnames(interpClean())
})
